...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        maybe_mkdir_p(self.epoch_output_folder)
        self.log_file = join(self.output_folder, "training_log_%d_%02.0d_%02.0d_%02.0d_%02.0d.txt" %
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.loss = self._build_loss() #self._build_loss()
...
#                                   weight_ce=1, weight_dice=1, ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
            loss  = AutoWeighted_DC_and_CE_and_Focal_loss({'batch_dice': self.configuration_manager.batch_dice, 'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp},
                                                          {},
...
### UU-Mamba (Uncertainty-Aware loss + SAM optimizer)
    def train_step(self, batch: dict) -> dict:
        data = batch['data']
        target = batch['target']
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('EMA pseudo Dice: ', np.round(self.logger.my_fantastic_logging['ema_fg_dice'][-1], decimals=4))
        self.print_to_log_file(
