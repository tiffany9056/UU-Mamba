...
class DC_and_topk_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, topk_kwargs, weight_topk=1, weight_dice=1, ignore_label=None):
        """
        Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        :param soft_dice_kwargs:
        :param topk_kwargs:
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        topk_loss = self.topk(net_output, target) \
            if self.weight_topk != 0 and (self.ignore_label is None or num_fg > 0) else 0

        result = self.weight_topk * topk_loss + self.weight_dice * dc_loss
        return result
...
class AutoWeighted_DC_and_CE_and_Focal_loss(nn.Module):
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        """
        if self.ignore_label is not None:
            assert target.shape[1] == 1, 'ignore label is not implemented for one hot encoded target variables (AutoWeighted_DC_and_CE_and_Focal_loss)'
            mask = target != self.ignore_label 
            target_dice = torch.where(mask, target, 0)
            num_fg = mask.sum()
        else:
